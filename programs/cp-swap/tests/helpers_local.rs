...
pub async fn setup_test_environment(program_id: Pubkey) -> TestEnv {
    let config = ProgramTestConfig::new_v2(
        true, // with_prover
        Some(vec![("raydium_cp_swap", program_id)]),
    );

    let mut rpc = LightProgramTest::new(config).await.unwrap();
    let payer = rpc.payer.insecure_clone();

    // Setup mock program data for the BPF Loader
...
/// Setup mock program data PDA for the BPF Loader.
pub fn setup_mock_program_data(
    ...
    // Create mock program data with upgrade authority set to payer
    // ProgramData account layout:
    // - discriminator: u32 (4 bytes) - must be 3 for ProgramData
    // - slot: u64 (8 bytes)
    // - upgrade_authority_address: Option<Pubkey> (1 + 32 bytes)
    ...
    data[13..45].copy_from_slice(payer.pubkey().as_ref()); // Authority pubkey
